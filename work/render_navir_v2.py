...
def split_line(draw, points, split_x, left_color, right_color, width):
    for a, b in zip(points[:-1], points[1:]):
        color = left_color if (a[0] + b[0]) / 2 < split_x else right_color
...
def render_brand_board():
    ...
    # Large brand-color field: a structural zone, not a small accent.
    d.rectangle((1240, 150, 1832, 900), fill=BURGUNDY)
    ...
    # Product sculpture
    d.ellipse((1280, 160, 1930, 810), outline=(137, 50, 71), width=76)
    prod = resize_h(PRODUCT, 780)
    im.paste(prod, (1245, 115), prod)
    d.text((1396, 834), "NAVIR S1", font=font(BOLD, 26), fill=WARM)
    d.text((1396, 874), "SOFT SILVER / BURGUNDY", font=font(MONO, 12), fill=(229, 205, 211))
    ...
    path = OUT / "NAVIR_Brand_Board_1.1_ColorField.png"
...
def render_kv():
    ...
    # Large burgundy field concentrates the invisible airflow into brand color.
    split_x = 1420
    d.polygon(((split_x, 0), (2400, 0), (2400, 2260), (1840, 2260), (split_x, 1960)), fill=BURGUNDY)

    # Monumental ring bridges the quiet and chromatic zones.
    d.ellipse((1060, 250, 2930, 2120), outline=(218, 210, 210), width=150)
    d.arc((1185, 375, 2805, 1995), 166, 314, fill=(236, 225, 226), width=12)
    ...
    split_line(d, back, split_x, (171, 57, 83), WARM, 18)
    split_line(d, back, split_x, WARM, (150, 62, 82), 5)
    ...
    split_line(d, front, split_x, BURGUNDY, WARM, 13)
    h1 = bezier_points((1090, 1810), (1510, 1510), (1860, 1430), (2460, 1210))
    h2 = bezier_points((1050, 1875), (1510, 1580), (1980, 1550), (2510, 1320))
    split_line(d, h1, split_x, (82, 81, 79), (235, 221, 224), 3)
    split_line(d, h2, split_x, BURGUNDY, WARM, 2)

    # Meta typography
    tracked(d, (150, 126), "NAVIR", font(BOLD, 46), GRAPHITE, 9)
    tracked(d, (1860, 144), "FLOW STUDY  01/03", font(MONO, 18), WARM, 2)
    d.line((150, 224, 2250, 224), fill=GRAPHITE, width=2)
    d.line((split_x, 224, 2250, 224), fill=WARM, width=2)
    ...
    path = OUT / "NAVIR_KV01_SHAPE_THE_AIR_v2_ColorField.png"
    im.save(path, quality=96)
    return im, path
